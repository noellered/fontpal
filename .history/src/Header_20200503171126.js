...
x
        ...
        const formatGroupLabel = data => (
            <div style={groupStyles}>
                <button onClick={this.hideItems}>  
                    <span>{data.label}</span>
                </button>
            </div>
        );

        const sansSerifFonts = this.state.sansSerif.map(family => ({value: family, label: family, fontname: family, styles: { display: this.state.showInfo ? "block" : "none" }}));
