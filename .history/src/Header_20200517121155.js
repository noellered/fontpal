...
class Header extends React.Component {
    ...
    render(){
        ...
        const sansSerifFonts = this.state.sansSerif.map(family => ({value: family, label: family, fontname: family, display: this.state.showSSInfo ? 'block' : 'none', id: family}));
        const serifFonts = this.state.serif.map(family => ({value: family, label: family, fontname: family, display: this.state.showSerifInfo ? 'block' : 'none', id: famlily}));
        const displayFonts = this.state.display.map(family => ({value: family, label: family, fontname: family, display: this.state.showDisplayInfo ? 'block' : 'none', id: family}));
        const handwritingFonts = this.state.handwriting.map(family => ({value: family, label: family, fontname: family, display: this.state.showHandwritingInfo ? 'block' : 'none', id: family}));
        const monospaceFonts = this.state.monospace.map(family => ({value: family, label: family, fontname: family, display: this.state.showMonoInfo ? 'block' : 'none', id: family}));
        ...
        return(
                <header>
                    ...
                    <div className="form-container">
                      <label>Body:</label>
                          <Dropdown defaultValue={{value: this.props.bodyValue, label: this.props.bodyValue}} onChange={this.props.onBodyChange} value={this.props.bodyLabel} options={groupedOptions} styles={customStyles} formatGroupLabel={formatGroupLabel} open={true}/>
                          <span style={{display: 'block', margin: '6px'}}/>
