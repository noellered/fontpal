...
class Header extends React.Component {
    ...
    render(){
        ...
        const formatGroupLabel = data => (
            <div>  
                <button style={groupStyles} onClick={this.hideItems} value={data.label}>{data.label} <FontAwesomeIcon icon={this.state.showSSInfo ? faCaretUp : faCaretSquareDown}/> </button>
            </div>
        );

        const sansSerifFonts = this.state.sansSerif.map(family => ({value: family, label: family, fontname: family, display: this.state.showSSInfo ? 'block' : 'none', icon: this.state.showSSInfo ? faCaretUp : faCaretSquareDown}));
