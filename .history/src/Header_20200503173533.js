...
class Header extends React.Component {
    ...
    hideItems = (e) => {
        ...
        if(e.target.value === "Sans-Serif"){
            //set sans serif display to none
        }
        this.setState(prevState => ({showInfo: !prevState}));
    }
    ...
    render(){
        ...
        const formatGroupLabel = data => (
            <div style={groupStyles}>
                <div>  
                    <button onClick={this.hideItems} value={data.label}>{data.label}</button>
                </div>
            </div>
        );

        const sansSerifFonts = this.state.sansSerif.map(family => ({value: family, label: family, fontname: family, display: this.state.showInfo ? 'block' : 'none'}));
