      ...
      determineIndex(options, choice) {
        let index = 0;
          ...
          for(let i=0; i < options.length; i++){
            while (index > 0) {
            index = options[i].options.map(i => i.fontname).indexOf(this.props.headerValue);
            }
            console.log("index is ", options[i].options.map(x => x.value).indexOf(this.props.headerValue));
        }
        return index;
      }
        ...
        return(
                <header>
                    <div className="form-container">
                      <label>Header:</label>
                        <Dropdown defaultValue={{value: this.props.headerValue, label: this.props.headerValue}}  onChange={this.props.onHeaderChange} value={groupedOptions[1].value} options={groupedOptions} styles={customStyles} formatGroupLabel={formatGroupLabel}/>
