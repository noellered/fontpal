...
//Fetches Google Fonts API and sets fonts state to array of all google font family names
  ...
  componentDidMount(){
    ...
    render(){
        ...
        let sansSerifFonts = this.state.sansSerif.map((family) => options=[{value: {family}, label: {family}}])}
        ...
        return(    
            ...
            <form className="textSelectors">
              <label>Choose a {textType} Family:</label>
              <Select value={this.props.value} onChange={this.props.onChange} options={this.state.sansSerif.map((family) => options=[{value: {family}, label: {family}}])}/>
            //     <optgroup label="Sans-Serif Fonts">
            //       {this.state.sansSerif.map((family) => <option key={family} value={family}> {family} </option>)}
            //     </optgroup>
            //     <optgroup label="Serif Fonts">
