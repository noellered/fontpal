...
class Header extends React.Component {
    ...
    //Fetches Google Fonts by popularity and sets fonts state to array of all google font family names - limited to 100 by default
      ...
      determineIndex(options, choice) {
        ...
        options.forEach((option, i)=>
          {
            console.log('logging', option.options.map(i => i.value).indexOf(this.props.headerValue))
            if (option.options.map(i => i.value).indexOf(this.props.headerValue) >= 1){
              index = option.options.map(i => i.value).indexOf(this.props.headerValue);
              console.log()
            }
          })
        return index;
    ...
    render(){
        ...
        const indexHeader = this.determineIndex(groupedOptions, this.props.headerValue);
        ...
        return(
                <header>
                    <div className="form-container">
                      <label>Header:</label>
                        <Dropdown defaultValue={{value: this.props.headerValue, label: this.props.headerValue}}  onChange={this.props.onHeaderChange} value={groupedOptions[0][indexHeader]} options={groupedOptions} styles={customStyles} formatGroupLabel={formatGroupLabel}/>
