...
//Fetches Google Fonts API and sets fonts state to array of all google font family names
  ...
  componentDidMount(){
    ...
    render(){
        ...
        const sansSerifFonts = this.state.sansSerif.map(family => ({value: family, label: family, style: {fontFamily: family}}));
        const serifFonts = this.state.serif.map(family => ({value: family, label: family, style: {fontFamily: family}}));
        ...
        return(
            <form className="textSelectors">
              ...
              <Select defaultValue={this.props.value} onChange={this.props.onChange} options={groupedOptions} getStyles={(name, whatever) => {console.log(name, whatever)}}/>
            </form>
